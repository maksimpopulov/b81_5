...
namespace GameOfLife.Tests
{
    ...
    public class IntegrationTests
    {
        ...
        [TestMethod]
        public void FullSimulation_TwoGliders_InteractAndChange()
        {
            var board = new Board(30, 30);
            board.LoadPattern(new[] { " * ", "  *", "***" }, 5, 5);
            board.LoadPattern(new[] { " * ", "  *", "***" }, 15, 15);

            var beforeClusters = board.FindClusters().Count;
            Assert.AreEqual(2, beforeClusters);
            ...
            var afterClusters = board.FindClusters().Count;
            // They may have merged or changed
            Assert.IsTrue(afterClusters >= 1);
        }
    }
}
